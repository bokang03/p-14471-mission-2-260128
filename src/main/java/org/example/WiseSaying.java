...
public class App {
    ...
    private int findIndexById(int id) {
        return IntStream
                .range(0, wiseSayings.size()) // 0부터 wiseSayings 크기 -1 까지, rangeClose()는 wiseSayings크기까지 간다.
                .filter((i) -> wiseSayings.get(i).getId() == id) // 조건문
                .findFirst() // 제일 처음 조건에 맞는 값 가져오기
                .orElse(-1); // 없을 경우 -1 return
    }
